...
static sal_l2cap_channel_t* find_channel_by_cid(uint16_t local_cid)
{
    for (int i = 0; i < L2CAP_MAX_CHANNELS; i++) {
        if (g_channels[i].in_use && g_channels[i].local_cid == local_cid) {
            return &g_channels[i];
        }
    }
    return NULL;
...
{
    for (int i = 0; i < L2CAP_MAX_CHANNELS; i++) {
        if (g_channels[i].in_use && &g_channels[i].le_chan == le_chan) {
            return &g_channels[i];
        }
    }
    return NULL;
...
static sal_l2cap_channel_t* alloc_channel(void)
{
    for (int i = 0; i < L2CAP_MAX_CHANNELS; i++) {
        if (!g_channels[i].in_use) {
            memset(&g_channels[i], 0, sizeof(sal_l2cap_channel_t));
            g_channels[i].in_use = true;
            return &g_channels[i];
        }
    }
    return NULL;
...
static sal_l2cap_server_t* find_server_by_psm(uint16_t psm)
{
    for (int i = 0; i < L2CAP_MAX_SERVERS; i++) {
        if (g_servers[i].in_use && g_servers[i].psm == psm) {
            return &g_servers[i];
        }
    }
    return NULL;
...
static sal_l2cap_server_t* alloc_server(void)
{
    for (int i = 0; i < L2CAP_MAX_SERVERS; i++) {
        if (!g_servers[i].in_use) {
            memset(&g_servers[i], 0, sizeof(sal_l2cap_server_t));
...
bt_status_t bt_sal_l2cap_send_packet(uint16_t local_cid, uint8_t* data, uint16_t size)
{
    sal_l2cap_channel_t* ch = find_channel_by_cid(local_cid);
    if (!ch) {
        return BT_STATUS_NOT_FOUND;
